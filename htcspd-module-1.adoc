...
==== pp: string length less than 5

.pp: string less than 5
[source,racket,lineos]
----
include::./01b-how-to-design-functions/pp01-str-less-than-5.rkt[]
----
...
[source,racket,lineos]
----
include::./01b-how-to-design-functions/pp02-boxify.rkt[]
----
...
[source,racket,lineos]
----
include::./01b-how-to-design-functions/pp03-double-error.rkt[]
----
...
[source,racket,lineos]
----
include::./01b-how-to-design-functions/qa03-larger-image.rkt[]
----


== 2: How to Design Data

It turns out that the *design of data drives the design of functions*.


=== cond expressions

If you have IFs inside true of false answers, perhaps `cond` is the way to go.

`cond` is a _multi-armed conditional_. It can have any number of cases all at the same level.

.vd: image aspect ratio with cond
[source,racket,lineos]
----
include::./02-how-to-design-data/vd01-cond-aspect-ratio.rkt[]
----

.vd: cond eval steps
[source,racket,lineos]
----
include::./02-how-to-design-data/vd02-cond-eval-steps.rkt[]
----

=== Data Definitions

.part of traffic simulation
[source,racket,lineos]
----
include::./02-how-to-design-data/vd03-part-traffic-sim.rkt[]
----

The function consumes a natural. Can I call it with 3 as well? What about 50, 200, etc?

If I pass 2 to the function it produces zero? Why? What the poop‽

==== program domain vs program

A light is red (program domain), but we may represent it with the value 0 in the program. We represent real things with data. Red light is represented with zero, for instance.

In the problem domain we have information, and translate that as data in the program.

TIP: *Data Definitions* describe how we are representing information as data.

.part of traffic simulation with data definition

[source,racket,lineos]
----
include::./02-how-to-design-data/vd04-traffic-light-datadef.rkt[]
----
...
- Template: template function for handling that kind/structure of data.
- Signature: tells the function consumes TLColor, and that means I cannot pass the function a 3 or 50 or 200, because a TLColor is either 1, or 2, or 3. And it just produces a TLColor.

A Data Definition also tells you all the things the function does not consume and does not produce, which is very important as well.
...
.data definition example: city name
[source,racket,lineos]
----
include::./02-how-to-design-data/vd05-datadef-city-name.rkt[]
----

=== HtDF With Non-Primitive Data

Functions that produce a boolean should have at least two tests: one that produces #true and one that produces #false.

Templates show what you can use, not what you must use. Sometimes you will delete parts of templates, and sometimes add extra things.

.HtDF with non-primiitve data
[source,racket,lineos]
----
include::./02-how-to-design-data/vd05-datadef-city-name.rkt[]
----
...
[source,racket,lineos]
----
include::./02-how-to-design-data/pp01-icon-too-tall.rkt[]
----

=== HtDF X Structure of Data Orthogonality

Orthogonal means “mostly independent”. HtDF and HtDD recipes are orthogonal, as we learn more forms of data, the recipes will continue to work mostly unchanged.
...
.vd: data def aisle seat
[source,racket,lineos]
----
include::./02-how-to-design-data/vd07-interv-seat-num-datadef.rkt[]
----
...
.vd: data def letter grade
[source,racket,lineos]
----
include::./02-how-to-design-data/vd08-letter-grade-enumeration.rkt[]
----
...
[source,racket,lineos]
----
include::./02-how-to-design-data/vd09-countdown-datadef.rkt[]
----

In a mixed data itemization template the type specific predicates must be guarded against being called on the wrong type of data.

.seat num data def
[source,racket,lineos]
----
include::./02-how-to-design-data/vd10-seat-num-itemization.rkt[]
----

Rules for simplifying templates:

1. If a given subclass is the last subclass of its type, we can reduce the test to just the guard, ie. `(number? arg)`.
2. If all remaining subclasses are of the same type, then we can eliminate all of the guards.

[source,racket,lineos]
----
include::./02-how-to-design-data/vd11-traffic-light.rkt[]
----
...
[source,racket,lineos]
----
include::./02-how-to-design-data/vd12-aisle-seat.rkt[]
----
...
[source,racket,lineos]
----
include::./02-how-to-design-data/vd13-bump-up-grade-enumeration.rkt[]
----

=== HtDF With Itemization

You could name the function `countdown->image` instead of `countdown-to-image`.

[source,racket,lineos]
----
include::./02-how-to-design-data/vd14-countdown-to-image-itemization.rkt[]
----
...
==== pp: demolish (enumeration)

[source,racket,lineos]
----
include::./02-how-to-design-data/pp02-demolish.rtk[]
----

==== pp: rocket descent (interval)

[source,racket,lineos]
----
include::./02-how-to-design-data/pp03-rocket-descent.rkt[]
----

=== qz: age healt problem

[source,racket,lineos]
----
include::./02-how-to-design-data/qz-age-healt-problem.rtk[]
----
...
- Analyze the program to identify constant information.

=== Program Through the Main Function

Here we use the wish-list technique to manage the overall design work.

Templates gives you pieces of the solutions but also steps of the process.
...
A wish-list entry has a signature, purpose, `!!!` and a stub. It is a promise to come back and finish this function later. The stubs in wish list entries can allow programs to at least partially run.

=== Working Through The Wish List

[source,racket,lineos]
----
include::./03a-how-to-design-worlds/vd01-cat.rkt[]
----

=== Improving a World Program — Add SPEED

Speed was an _unidentified_ constant that our program used blindly.

The Analysis is a *model* of the program. It correctly describes the structure of the program (but in less detail), easing our task of planning our work on the program.

=== Improving a World Program — Add Key Handler

Templating and testing large enumerations is also talked about in this part.

The white-box test takes advantage of knowing that the `cond` has just two cases.

For large enumerations the template will have `cond` clauses for all the particular cases, and an `else` clause for everything else.

==== cat animation, key event

.cat animation with key event
[source,racket,lineos]
----
include::./03a-how-to-design-worlds/vd03-cat-kevt.rkt[]
----

==== cat animation, mouse event

.cat animation with mouse event
[source,racket,lineos]
----
include::./03a-how-to-design-worlds/vd04-cat-mevt.rkt[]
----
...
[source,racket,lineos]
----
include::./03a-how-to-design-worlds/pp01-contdown-animation.rtk[]
----
...
[source,racket,lineos]
----
include::./03a-how-to-design-worlds/pp02-trafficlight-animation.rkt[]
----

We did not use MTS in this exercise, and they didn't use it in their solution either.


== Compund Data

Data data naturally belong together.

=== define-struct

[source,racket,lineos]
----
;; pos is the structure name, x and y are the fields.
(define-struct pos (x y))

(define P1 (make-pos 3 6))  ; make-pos is the constructor.
...
Defining a struct doesn't produce a value.

=== Compound Data Definitions

.hockey player data definition example
[source,racket,lineos]
----
include::./03b-compound-data/vd01-hockey-player.rkt[]
----
...
[source,racket,lineos]
----
include::./03b-compound-data/pp01-most-recent-movie.rkt[]
----

===== student add to list
[source,racket,lineos]
----
include::./03b-compound-data/pp02-student-grade-allergies.rkt[]
----
...
.cowabunga analisys
image:./imgs/018-cowabunga-analisys.jpg[cowabung analisys]

You don't want a function (`render-cow` in this case) to do two things like:

1. choose which image to place
2. place the image on proper position on MTS

In `render-cow` we will _whish_ for a `choose-image` *helper function*.

Helper functions need tests as well.

.cowabunga, on-tick, to-draw and on-key
[source,racket,lineos]
----
include::./03b-compound-data/vd02-cowabunga.rkt[]
----
